...
## Phase 26: Wide Screen + Button-only Control Fix
- [x] ゲーム内部解像度を縦長過多から是正（横幅活用しやすい比率へ調整）
- [x] 盤面レイアウトを上寄せ・最大化し、左右余白と下部空白を圧縮
- [x] CSSでスマホ幅いっぱい表示へ変更（max-width制限解除・余白削減）
- [x] Title/StageSelect/Game を mobileControls 前提へ統一（キーボード/盤面タップ依存を除去）
