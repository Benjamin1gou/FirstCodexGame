...
FirstCodexGame は **TypeScript + Phaser + Vite** で作られた、静的配信向けの2D逆タワーディフェンス型パズルです。GB風（緑系）UIで、スマホ縦画面プレイを前提に設計されています。
プレイヤーは魔王軍の参謀として、勇者に直接攻撃せず罠配置で誘導・撃退します。

## 操作方法
- 罠選択: キーボード `1-6` / 画面下の罠カード
- 罠配置: 盤面タップ（盤面外タップは無効）
- 実行: `Enter` / 実行ボタン
